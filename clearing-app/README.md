...
- **统一日互斥边界 + 关账竞争门**：`settlement_day_lock`（每日一行，FOR UPDATE）与
  `settlement_day_gate`（每次关账尝试一行，`active_token` 唯一约束保证同一日至多一个 PENDING）配合：
  - 关账分两段——独立短事务**持有日行锁**登记 PENDING 门后释放，主事务再取日行锁建快照；
    锁行首次并发创建在 REQUIRES_NEW 内独立回滚（不污染调用方事务），失败可显式重试；
  - 关账主事务与确认事务在同一把日行锁上竞争：关账先持锁则置门 CLOSED 并提交，确认后拿到锁见 CLOSED 报表→409；
    确认先持锁则把 PENDING 门置 SUPERSEDED 并提交，关账 claim 时→409。**竞争窗口恰好一方成功**，
    失败方整体回滚（无 CONFIRMED 批次/债权清偿/快照/排除/指令半成品）；
  - 闩锁测试用 {@link com.treasury.clearing.service.ConcurrencyHooks} 精确控制关账登记意图、
    关账主事务取锁、确认取锁三点，强制覆盖关账先胜、确认先胜、首次锁行不存在三种交错。
...
```bash
cd backend && mvn test                 # 35 个测试：引擎 + 服务集成 + 守恒 + 撤销/更正四眼 + 关账/再开账 + 关账/确认唯一胜者并发
