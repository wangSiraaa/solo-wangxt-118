...
- **统一日互斥边界**：`settlement_day_lock`（每个结算日一行）覆盖“空快照 + 状态切换”。
  关账、再开账、**确认生效**、撤销末审、差额更正末审都在各自事务**最开始**对当日锁行取
  `SELECT ... FOR UPDATE`（锁行经独立事务惰性创建），随后才检查关账状态/审批中批次、做汇总或确认。
  因此并发“关账 vs 确认”恰好一方成功：关账先持锁则确认 409 且整体回滚（无半成品批次/债权改动）；
  确认先持锁则关账等待，其提交后关账重查必能看到新批次，快照完整不漏。日锁只串行化并发窗口，
  跨事务仍以 CLOSED/REOPEN_PENDING 状态判定是否已关账。
...
```bash
cd backend && mvn test                 # 32 个测试：引擎 + 服务集成 + 守恒 + 撤销/更正四眼 + 关账/再开账 + 关账/确认并发互斥
